...
GOOS=linux GOARCH=amd64 go build -o "$ROOT/bin/linux/"
GOOS=windows GOARCH=amd64 go build -o "$ROOT/bin/windows/img_to_pix.exe"
GOOS=darwin GOARCH=amd64 go build -o "$ROOT/bin/macos_x86/"
GOOS=darwin GOARCH=arm64 go build -o "$ROOT/bin/macos_arm/"
